    ...
    where TEntity : class
{
    ...
    public EfCoreQueryProvider(IDbContextFactory<TDbContext> dbContextFactory)
    {
    ...
    public TResult ExecuteAsync<TResult>(Expression expression, CancellationToken cancellationToken = default)
    {
        ...
        var result = ((IAsyncQueryProvider)queryable.Provider).ExecuteAsync<TResult>(convertedExpression, cancellationToken);
        var genericTypeArgument = result?.GetType().GetInterfaces().First(x => x.IsGenericType && x.GetGenericTypeDefinition() == typeof(IAsyncEnumerable<>)).GenericTypeArguments.First();

        var genericType = typeof(EfCoreAsyncEnumerable<>).MakeGenericType(genericTypeArgument ?? throw new InvalidOperationException("Generic type argument is missing in Expression"));
        return (TResult)(Activator.CreateInstance(genericType, db, result) ?? throw new InvalidOperationException("Generic type argument is missing in Expression"));
    }
    ...
    /// <summary>
    /// Visitor for the <see cref="ExpressionVisitor"/> for replacing expression from <see cref="EfCoreQueryable{T}"/> to the given expression
    /// </summary>
