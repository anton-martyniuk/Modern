...
[Generator]
public class ModernServiceGenerator : ISourceGenerator
{
    private const string ServiceAttributeName = nameof(ModernServiceAttribute);
    private const string EntityDboTypeMemberName = nameof(ModernServiceAttribute.EntityDboType);
    private const string ServiceMemberName = nameof(ModernServiceAttribute.ServiceName);
    ...
    public void Execute(GeneratorExecutionContext context)
    {
        ...
        foreach (var classDeclaration in receiver.ServiceClasses)
        {
            ...
            context.AddSource($"{serviceName}_service_gen.cs", SourceText.From(source, Encoding.UTF8));
        }
    }
        ...
        string idType, string serviceName)
    {
        ...
        sb.AppendLine("using Modern.Services.DataStore;");
        ...
        sb.AppendLine($"public partial class {serviceName} : ModernService<{entityDtoType}, {entityDboType}, {idType}>, I{serviceName}");
        sb.AppendLine("{");
        sb.AppendLine("     ///<summary>Initializes a new instance of the class</summary>");
        sb.AppendLine($"    public {serviceName}(IModernRepository<{entityDboType}, {idType}> repository, ILogger<ModernService<{entityDtoType}, {entityDboType}, {idType}>> logger)");
        sb.AppendLine("        : base(repository, logger)");
        sb.AppendLine("    {");
        sb.AppendLine("    }");
        sb.AppendLine("}");
