...
[Generator]
public class ModernControllerGenerator : ISourceGenerator
{
    private const string ControllerAttributeName = nameof(ModernControllerAttribute);
    private const string CreateRequestTypeMemberName = nameof(ModernControllerAttribute.CreateRequestType);
    private const string UpdateRequestTypeMemberName = nameof(ModernControllerAttribute.UpdateRequestType);
    private const string EntityDboTypeMemberName = nameof(ModernControllerAttribute.EntityDboType);
    private const string ApiRouteMemberName = nameof(ModernControllerAttribute.ApiRoute);
    private const string ControllerMemberName = nameof(ModernControllerAttribute.ControllerName);
        ...
        string idType, string createRequestType, string updateRequestType, string controllerName, string apiRoute)
    {
        ...
        sb.AppendLine($"public partial class {controllerName} : ModernController<{createRequestType}, {updateRequestType}, {entityDtoType}, {entityDboType}, {idType}>");
        sb.AppendLine("{");
        sb.AppendLine("     ///<summary>Initializes a new instance of the class</summary>");
        sb.AppendLine($"    public {controllerName}(IModernService<{entityDtoType}, {entityDboType}, {idType}> service)");
...
public class RepositorySyntaxReceiver : ISyntaxReceiver
{
    ...
    public void OnVisitSyntaxNode(SyntaxNode syntaxNode)
    {
        ...
        if (classDeclarationSyntax.AttributeLists.Any(a => a.Attributes.Any(x => x.Name.ToString().Contains("ModernController"))))
        {
